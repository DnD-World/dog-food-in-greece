...
export function downloadProductsAsCSV(products: DogProduct[], filename = 'greece_dog_products_catalog.csv') {
  ...
  const rows = products.map((p) => [
    ...
    escapeCSV(p.availableSizes ? p.availableSizes.join('; ') : p.packageSize),
    p.msrpEuros.toFixed(2),
    p.pricePerKg.toFixed(2),
    escapeCSV(p.itemImageUrl),
    escapeCSV(p.ingredientsImageUrl),
    escapeCSV(p.drivePackImageUrl || ''),
    escapeCSV(p.driveIngredientsImageUrl || ''),
    escapeCSV(p.officialProductUrl),
    escapeCSV(p.skroutzUrl),
    escapeCSV(p.bestPriceUrl),
    escapeCSV(p.certifyingBody),
    escapeCSV(p.hasIndependentLabReport ? 'Verified Independent 3rd-Party Lab' : 'Manufacturer In-House QA (Self-Declared)'),
    escapeCSV(p.hasIndependentLabReport && p.labReportUrl ? p.labReportUrl : ''),
    ...
    p.isRealLifeVerified ? 'VERIFIED' : 'SYNTHETIC_ESTIMATE',
    escapeCSV(p.verificationSourceType || ''),
    escapeCSV(p.verificationMethod || ''),
    escapeCSV(p.verificationDate || ''),
    escapeCSV(p.verificationSourceUrl || ''),
  ]);
