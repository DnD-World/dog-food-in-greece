...
import { PawPrint, Bone, Utensils, ChefHat, Sparkles, Store, ShieldCheck } from 'lucide-react';

export default function App() {
  // Navigation tabs: all, food, treats, recipes
  const [activeTab, setActiveTab] = useState<'all' | 'food' | 'treats' | 'recipes'>('all');

  // Auth state for Google Workspace / Sheets
  ...
  const availableBrands = useMemo(() => {
    const set = new Set(GREECE_DOG_PRODUCTS.map((p) => p.brand));
    return Array.from(set).sort();
  }, []);

  // Filtered products list
  const filteredProducts = useMemo(() => {
    return GREECE_DOG_PRODUCTS.filter((p) => {
      // Tab constraint
      if (activeTab === 'food') {
  ...
  const totalFoodCount = useMemo(
    () => GREECE_DOG_PRODUCTS.filter((p) => p.productType === 'Dry Food' || p.productType === 'Wet Food').length,
    []
  );
  const totalTreatsCount = useMemo(
    () => GREECE_DOG_PRODUCTS.filter((p) => p.productType === 'Treats' || p.productType === 'Dental Chews').length,
    []
  );
  const genericSupermarketCount = useMemo(
    () => GREECE_DOG_PRODUCTS.filter((p) => p.channelCategory === 'Supermarket Generic').length,
    []
  );
      ...
      <header className="border-b border-zinc-800/80 bg-zinc-950/90 backdrop-blur-md sticky top-0 z-30">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center gap-2 border-t border-zinc-850 pt-2 pb-2 overflow-x-auto">
          ...
          >
            ...
            All Products ({GREECE_DOG_PRODUCTS.length})
          </button>
